...
namespace EjercicioClase18
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            SqlConnection conexion = new SqlConnection(Properties.Settings.Default.Conexion); //Establezco una conexion al SQL Server. Properties.Settings.Default.Conexion -> hace referencia a la conexion establecida en los settings del proyecto.
            ...
            #region Agregar/Actualizar/Borrar DataBase
            //Agrego tele a la DataBase
            Televisor televisorAgregar = new Televisor(1248, "Samsung", 15000, 32, "Japon");
            Console.WriteLine(televisorAgregar.Insertar());

            //Modifico tele a la DataBase
            Televisor televisorModificar = new Televisor(1248, "Phillips", 47500, 56, "Rusia");
            Televisor.Modificar(televisorModificar);

            //Borro tele de la DataBase
            Televisor.Borrar(televisorModificar);
            #endregion

            List<Televisor> traerTodos = Televisor.TraerTodos();

            Televisor traerUno = Televisor.TraerUno(1245);
            Console.WriteLine("{0}-{1}-{2}-{3}-{4}", traerUno.id, traerUno.marca, traerUno.precio, traerUno.pulgadas, traerUno.pais);
